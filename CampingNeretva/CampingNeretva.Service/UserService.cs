    ...
    public class UserService : BaseCRUDService<UserModel, UserSearchObject, User, UserInsertRequest, UserUpdateRequest>, IUserService
    {
        ...
        private async Task GenerateRecommendationsForUser(int userId)
        {
            ...
            var parcelIds = context.Parcels.Select(p => p.ParcelId)
                ...
                {
                    ItemId = itemId,
                    Score = predictionEngineParcel.Predict(new ReservationData
                    {
                        UserId = userId,
                        ItemId = itemId,
            ...
            var activityIds = context.Activities.Select(a => a.ActivityId)
                ...
                {
                    ItemId = itemId,
                    Score = predictionEngineActivity.Predict(new ReservationData
                    {
                        UserId = userId,
                        ItemId = itemId,
                        NumberOfPeople = numberOfPeople,
            ...
            var rentableItemIds = context.RentableItems.Select(ri => ri.ItemId)
                ...
                {
                    ItemId = itemId,
                    Score = predictionEngineRentable.Predict(new ReservationData
                    {
                        UserId = userId,
                        ItemId = itemId,
                        NumberOfPeople = numberOfPeople,
            ...
            {
                UserId = userId,
                ParcelId1 = parcelIds.ElementAtOrDefault(0) > 0 ? parcelIds.ElementAtOrDefault(0) : (int?)null,
                ParcelId2 = parcelIds.ElementAtOrDefault(1) > 0 ? parcelIds.ElementAtOrDefault(1) : (int?)null,
                ParcelId3 = parcelIds.ElementAtOrDefault(2) > 0 ? parcelIds.ElementAtOrDefault(2) : (int?)null,
                ActivityId1 = activityIds.ElementAtOrDefault(0) > 0 ? activityIds.ElementAtOrDefault(0) : (int?)null,
                ActivityId2 = activityIds.ElementAtOrDefault(1) > 0 ? activityIds.ElementAtOrDefault(1) : (int?)null,
                RentableItemId1 = rentableItemIds.ElementAtOrDefault(0) > 0 ? rentableItemIds.ElementAtOrDefault(0) : (int?)null,
                RentableItemId2 = rentableItemIds.ElementAtOrDefault(1) > 0 ? rentableItemIds.ElementAtOrDefault(1) : (int?)null
            };
